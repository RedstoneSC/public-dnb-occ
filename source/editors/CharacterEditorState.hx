...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBarBG = new FlxSprite(30, FlxG.height - 75).loadGraphic(Paths.image('healthBars/' + ClientPrefs.healthBarTexture.toLowerCase(), "shared"));
	...
	function reloadBGs()
	{
		...
		var bg:FlxSprite = new FlxSprite(-600 + OFFSET_X - playerXDifference, -300).loadGraphic(Paths.image('stageback', "shared"));
		bg.scrollFactor.set(0.9, 0.9);
		bgLayer.add(bg);

		var stageFront:FlxSprite = new FlxSprite(-650 + OFFSET_X - playerXDifference, 500).loadGraphic(Paths.image('stagefront', "shared"));
	...
	"image": "characters/realbambinotfake",
			"position": [
				0,
				0
			],
	...
	function reloadGhost()
	{
		...
		ghostChar.scale.x = char.scale.x; // much better to imo
		ghostChar.scale.y = char.scale.y;
