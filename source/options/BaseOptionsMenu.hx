...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		descText = new FlxText(50, 600, 1190, "", 32);
		...
		{
			var optionText:Alphabet = new Alphabet(0, 70 * i, optionsArray[i].name, (optionsArray[i].variable == "dummy") ? true : false, false);
			...
			else if (optionsArray[i].variable != "dummy")
			{
				optionText.x -= 80;
				optionText.xAdd -= 80;
				...
				if (controls.UI_LEFT || controls.UI_RIGHT)
				{
					...
					if (holdTime > 0.5 || pressed)
					{
						if (pressed)
						{
							...
							curOption.change();
							if (curOption.name.trim() != "Hitsound Volume:")
								FlxG.sound.play(Paths.sound('scrollMenu'));
						}
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (curSelected >= optionsArray.length)
			curSelected = 0;
		holdValue = optionsArray[curSelected].getValue();
