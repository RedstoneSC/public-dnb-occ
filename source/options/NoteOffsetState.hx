...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		FlxG.sound.pause();
		// Stage
		var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('stageback', "shared"));
		bg.scrollFactor.set(0.9, 0.9);
		add(bg);

		var stageFront:FlxSprite = new FlxSprite(-650, 500).loadGraphic(Paths.image('stagefront', "shared"));
		...
		var diff = FlxG.random.float(-ClientPrefs.sickWindow, ClientPrefs.sickWindow);
		diff = flixel.math.FlxMath.roundDecimal(diff, 2);
		rating = new FlxSprite().loadGraphic(Paths.image('sick'));
		rating.cameras = [camHUD];
		rating.setGraphicSize(Std.int(rating.width * 0.7));
		rating.updateHitbox();
		rating.antialiasing = ClientPrefs.globalAntialiasing;
		msTiming = new FlxText(0, 0, 0, '${diff} ms');
		msTiming.setFormat(Paths.font('comic.ttf'), 34, 0xFF97FFFF, LEFT, OUTLINE, 0xFF000000);
		...
		for (i in 0...seperatedScore.length + 1)
		{
			...
			{
				var numScore:FlxSprite = new FlxSprite(43 * i, 70).loadGraphic(Paths.image('num' + seperatedScore[i]));
		...
		add(blackBox);
		moveText = new FlxText(0, FlxG.height * 0.92, FlxG.width, "Arrow Keys - Rating Offset, WASD - Numbers Offset, IJKL - MS Text Offset", 22);
		moveText.setFormat(Paths.font("comic.ttf"), 28, FlxColor.WHITE, CENTER, OUTLINE, 0xFF000000);
		moveText.scrollFactor.set();
		moveText.cameras = [camHUD];
		moveText.borderSize = 3;
		add(moveText);
	...
	var holdingObjectType:Null<HoldingObject> = null;
	...
	override public function update(elapsed:Float)
	{
		...
		if (onComboMenu)
		{
			...
			// probably there's a better way to do this but, oh well (well well).
			if (FlxG.mouse.justPressed)
			{
				...
				else if (startMousePos.x - msTiming.x >= 0
					&& startMousePos.x - msTiming.x <= msTiming.width
					&& startMousePos.y - msTiming.y >= 0
					&& startMousePos.y - msTiming.y <= msTiming.height)
				{
					holdingObjectType = MS;
					FlxG.mouse.getScreenPosition(camHUD, startMousePos);
			...
			if (holdingObjectType != null)
			{
				if (FlxG.mouse.justMoved)
				{
					...
					var addNum:Int = holdingObjectType == COMBO ? 2 : holdingObjectType == MS ? 4 : 0;
		...
		if (controls.BACK)
		{
			...
			CustomFadeTransition.nextCamera = camOther;
			MusicBeatState.switchState(new OptionsState(OptionsState.instance.inPlayState));
	...
	function repositionCombo()
	{
		...
		comboNums.y += 60 - ClientPrefs.comboOffset[3];

		msTiming.screenCenter();
		msTiming.x = coolText.x + 80 + ClientPrefs.comboOffset[4];
		msTiming.y -= -15 + ClientPrefs.comboOffset[5];
		reloadTexts();
	}
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch (i)
			{
				...
				case 5:
					dumbTexts.members[i].text = 'MS Text Offset:';
				case 6:
	...
	function updateMode()
	{
		...
		msTiming.visible = onComboMenu;
		moveText.visible = onComboMenu;
