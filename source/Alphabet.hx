...
class Alphabet extends FlxSpriteGroup
{
	...
	public var inFreeplay = false;
	public var inOST = false;
	...
	public var hasDialogueSounds = true;
	...
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1)
	{
		...
		this.typed = typed;
		isBold = bold;
		if (this.text != "" && this.text.toLowerCase() != "[redacted]")
		{
			if (typed)
			{
		...
		else if (this.text.toLowerCase() == "[redacted]")
		{
			var redacted = new FlxSprite(0, -5);
			redacted.frames = Paths.occurSparrow("expungement");
			redacted.animation.addByPrefix("glitch", "manifest", 24, true);
			redacted.animation.play("glitch");
			add(redacted);
		}
		else
		{
	...
	public var dialogueSoundName = "dialogue";
	...
	public function startTypedText(speed:Float):Void
	{
		...
		if (speed <= 0)
		{
			...
			if (dialogueSound != null)
				dialogueSound.stop();
			if (hasDialogueSounds)
			{
				switch (dialogueSoundName)
				{
					case "redbambi" | "bambi" | "cosmic":
						dialogueSound = FlxG.sound.play(Paths.soundRandom("dialogueTextSounds/" + dialogueSoundName, 1, 3));
					default:
						dialogueSound = FlxG.sound.play(Paths.sound("dialogueTextSounds/" + dialogueSoundName));
				}
			}
		}
		else
		...
		if (loopNum <= splitWords.length && splitWords[loopNum] != null)
		{
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				if (tmr != null)
				{
					...
					if (hasDialogueSounds)
					{
						if (dialogueSoundName.startsWith("redbambi")
							|| dialogueSoundName.startsWith("bambi")
							|| dialogueSoundName.startsWith("cosmic")
							|| dialogueSoundName.startsWith("darkenu")) // what? ineffecient?
							dialogueSound = FlxG.sound.play(Paths.soundRandom("dialogueTextSounds/" + dialogueSoundName, 1, 3));
						else
							dialogueSound = FlxG.sound.play(Paths.sound("dialogueTextSounds/" + dialogueSoundName));
					}
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			if (inFreeplay)
			{
				y = FlxMath.lerp(y, (scaledY * 112.5) + (FlxG.height * 0.45), 0.20 / (openfl.Lib.application.window.frameRate / 60));

				x = FlxMath.lerp(x, Math.exp(Math.abs(scaledY * 0.8)) * -70 + (FlxG.width * 0.275), 0.20 / (openfl.Lib.application.window.frameRate / 60));
			...
			{
				y = FlxMath.lerp(y, (scaledY * 115) + (FlxG.height * 0.48), 0.20 / (openfl.Lib.application.window.frameRate / 60));
				x = FlxMath.lerp(x, ((FlxG.width - width) / 2) + (Math.abs(scaledY) * 76), 0.20 / (openfl.Lib.application.window.frameRate / 60));
				if (width > 900)
					x -= 10;
				x += 3 * 1.05 * (Math.abs(scaledY) * 0.9);
				if (targetY == 0 && width <= 900)
					x -= 30;
				x -= 5;
			}
			else
...
class AlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, textSize:Float)
	{
		...
		setGraphicSize(Std.int(width * (textSize / 1.05)));
		updateHitbox();
		this.textSize = textSize;
