...
{
	public static var gameVersion:String = "V1.0"; // "1.0";
	public static var psychEngineVersion:String = '1.0.0'; // This is also used for Discord RPC -- not anymore :joy:
	...
	var optionShit:Array<String> = ['story_mode', 'freeplay', 'awards', 'donate', 'options', 'more'];
	...
	var curKey = 0;
	var keysHa = [FlxKey.H, FlxKey.E, FlxKey.H, FlxKey.E, FlxKey.H, FlxKey.E, FlxKey.H, FlxKey.A];
	var curKeyHa = 0;
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			var menuItem:FlxSprite = new FlxSprite(135 * (i + 1) + offset + ((i == 5) ? 60 : 0), (i * 140) + offset);
	...
	function onKeyPress(e:openfl.events.KeyboardEvent)
	{
		var eventKey:FlxKey = e.keyCode;
		if (eventKey == keys[curKey])
		{
			curKey++;
			if (curKey == 4)
			{
		...
		#if !STREAMER
		else if (eventKey == keysHa[curKeyHa])
		{
			curKey = 0;
			curKeyHa++;
			if (curKeyHa == 8)
			{
				...
				if (!ClientPrefs.songsLoaded.exists("heheheha"))
				{
					openSubState(new UnlockedNewSongSubState("Heheheha", "clashroyale"));
					for (menuItem in menuItems.members)
					{
						menuItem.animation.stop();
					}
					inSubstate = true;
				}
			}
	...
	{
		var chance = FlxG.random.int(0, bgs.length - 1);
		return Paths.image('mainMenubackgrounds/' + bgs[chance]);
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin && !inSubstate)
		{
			...
			if (controls.BACK)
			{
				...
				FlxG.stage.removeEventListener(openfl.events.KeyboardEvent.KEY_DOWN, onKeyPress);
			}
			...
			#if (desktop && !release)
			else if (FlxG.keys.anyJustPressed([SEVEN]))
			{
				selectedSomethin = true;
